...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/creator/:username', async (req: Request, res: Response) => {
    try {
  ...
  app.put('/api/creator', authenticateToken, async (req: Request, res: Response) => {
    try {
  ...
  app.get('/api/dashboard', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const uniqueSupporters = new Set(messages.map((m: any) => m.supporterName)).size;
      const totalAmount = messages.reduce((sum: number, m: any) => sum + (m.amount || 0), 0);
    ...
    try {
      ...
      if (amount && amount > 0) {
        ...
        if (goals.length > 0) {
          ...
          const incompleteGoal = goals.find((g: any) => g.currentAmount < g.targetAmount);
          if (incompleteGoal) {
  ...
  app.put('/api/goals/:goalId', authenticateToken, async (req: Request, res: Response) => {
    try {
  ...
  app.delete('/api/goals/:goalId', authenticateToken, async (req: Request, res: Response) => {
    try {
      const { goalId } = req.params;
